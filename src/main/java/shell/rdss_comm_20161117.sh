...
#		解压投产包
	...
	mysql -h127.0.0.1 -u${mysql_user} -p${mysql_pwd} RDS_DB < mysql_init_RDS_DB.sql
	echo "1/1 mysql -h127.0.0.1 -u${mysql_user} -p${mysql_pwd} RDS_DB < mysql_init_RDS_DB.sql 执行完毕"
...
# su - weblogic -c sqlplus $autekDbConnect @/oper/install/update$curdate/AutekDbList.sql|tee -a $installAutekDblog
	oper_date="20161118"  #xlx#
	cd /oper/install/${oper_date}/sql
	chmod -R 777 *
# sh redis_ini.sh redis_init_20160422 0 11.1.3.60 6379
	sh redis_ini.sh redis_init_MPC 0 ${redis_host} 6379
	echo "1/1 redis_init_MPC 执行完毕!"
...
#		解压投产包
		...
		cd /weblogic/rds_app/rds_rde/config
		su - weblogic -c "cd /weblogic/rds_app/rds_rde/config;cp -r scheduler.xml scheduler.xml20161118"
		su - weblogic -c "cd /weblogic/rds_app/rds_rde/config;cp -r trans_elemet_mapping.txt trans_elemet_mapping.txt20161118"
		su - weblogic -c "cd /oper/install/${oper_date}/rds_app/rds_rde/config;cp -r scheduler.xml /weblogic/rds_app/rds_rde/config"
		su - weblogic -c "cd /oper/install/${oper_date}/rds_app/rds_rde/config;cp -r trans_elemet_mapping.txt /weblogic/rds_app/rds_rde/config"
		su - weblogic -c "cd /weblogic/rds_app/rds_dwb/lib;cp -r packet.jar ../packet.jar.20161118"
		su - weblogic -c "cd /oper/install/${oper_date}/rds_app/rds_dwb/lib;cp -r packet.jar /weblogic/rds_app/rds_dwb/lib"
#		rds_app目录递归赋权限
